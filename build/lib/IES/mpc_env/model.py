...
class EnergyHubEnv:
    def __init__(self, 
        ...
        self.electricity_market  = electricity_market(self.components, self.station_id, self.pricing)
    ...
    def variables_def(self, model, T):
        ...
        self.electricity_market.variables_def(model, T)
    ...
    def model(self, T, model):
        ...
        self.electricity_market.model(T, model)
        ...
        # 电力平衡  
        model.addConstrs( (self.electricity_market.P_g_sell[t] + self.hydrogen_storage.P_el[t] + self.electrical_storage.P_bssc[t] + self.energy_simulation.non_shiftable_load[self.time_step + t]) 
                            <= (self.electricity_market.P_g_buy[t] +  self.energy_simulation.solar_generation[self.time_step + t] + self.hydrogen_storage.P_fc[t] + self.electrical_storage.P_bssd[t]) for t in range(T))     
        ...
        ### Objective function
        obj = self.hydrogen_storage.cost(T) + self.electrical_storage.cost(T) + self.heating_storage.cost(T) + self.cooling_storage.cost(T) + self.electricity_market.cost(T) + self.hydrogen_market.cost(T) + self.absorption_chiller.cost(T)
    ...
    def step(self, actions = None):
        ...
        self.electricity_market.step(actions)
    ...
    @property
    def get_reward(self):
        return self.electricity_market.get_reward + self.hydrogen_market.get_reward + self.electrical_storage.get_reward + self.hydrogen_storage.get_reward + self.cooling_storage.get_reward + self.heating_storage.get_reward + self.absorption_chiller.get_reward

    def update_state(self):
        self.states = {}
        # TODO 更新weather, energy simulation, price, 获取最新的states
        for observation in self.components:
            if any(key in observation for key in ['month','hour', 'day_type','load', 'demand', 'generation']):
                self.states[observation] = getattr(self.energy_simulation, observation)[self.time_step]
            elif 'electrical_storage' in observation:
                self.states[observation] = getattr(self, 'electrical_storage').traces.soc[self.time_step]
            elif 'hydrogen_storage' in observation:
                self.states[observation] = getattr(self, 'hydrogen_storage').traces.soc[self.time_step]
            elif 'heating_storage' in observation:
                self.states[observation] = getattr(self, 'heating_storage').traces.soc[self.time_step]
            elif 'cooling_storage' in observation:
                self.states[observation] = getattr(self, 'cooling_storage').traces.soc[self.time_step]
            elif 'power_market' in observation:
                self.states[observation] = self.pricing.electricity_price[self.time_step]
            elif 'hydrogen_market' in observation:
                self.states[observation] = self.hydrogen_market.traces.hydr_purc[-1]
            elif 'direct_solar_irradiance' in observation:
                self.states[observation] = self.solar_thermal_collector.P_solar_heat[self.time_step]
            elif 'absorption_chiller':
                self.states[observation] = self.absorption_chiller.traces.g_AC[-1]
            else:
                self.states[observation] = getattr(self.weather, observation)[self.time_step] 
